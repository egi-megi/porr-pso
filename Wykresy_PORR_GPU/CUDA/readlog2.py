...
mypath = os.getcwd()
mypath = os.path.join(mypath, "wykresy","CUDA")
onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f)) and "logFile" in f]

content = []

id= 0

with open(os.path.join(mypath,onlyfiles[id])) as f:
    content = [(line.rstrip()).split() for line in f]  


fileName = os.path.splitext(onlyfiles[id])[0]    
file_params=fileName.split('_')
particles = file_params[3]
dim = int(file_params[5])

costs = [float(x[1]) for x in content]
iterations = range(0,len(costs))

plt.figure()
myTitle = f"{lib}, {alg}, {task}: n={dim}, l.czastek={particles}, stop={stop} - wartość f.kosztu"
...
# plt.yscale("log")
plt.title(myTitle)
plt.savefig(f"{lib}_{alg}_{task}_koszt_linear.png",dpi=200)  
